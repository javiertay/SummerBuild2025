...
const AddEntryModal = ({ onClose, onSubmit }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "resume" || name === "commentsFile") {
      setFormData({ ...formData, [name === "resume" ? "resume" : "comments"]: files[0] });
    } else {
      ...
      <div className="bg-white w-full max-w-xl p-6 rounded-xl shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" className="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 text-nowrap">Add Entry</button>
          </div>
        </form>
      </div>
    </div>
